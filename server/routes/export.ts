...
import { ExportService, BatchExportSettings, highResExportService, HighResExportRequest, HighResExportProgress } from '../services/exportService';
...
export function registerExportRoutes(app: Express): void {
  ...
  /**
   * Async High-Resolution Export Endpoint
   * 
   * Starts an asynchronous export and returns an exportId for polling progress.
   * Useful for large tiled exports that may take a long time.
   */
  app.post('/api/export/high-resolution/async', async (req, res) => {
    try {
      const validationResult = HighResExportSchema.safeParse(req.body);
      ...
      console.log(`[HighRes Async Export] Starting async export: ${request.artboard.width}x${request.artboard.height} @ ${request.exportSettings.dpi || request.artboard.dpi} DPI`);
      
      const result = await highResExportService.startAsyncExport(request);
      
      res.json({
        success: true,
        exportId: result.exportId,
        needsTiling: result.needsTiling,
        totalTiles: result.totalTiles,
        message: result.needsTiling 
          ? `Export started with ${result.totalTiles} tiles. Poll /api/export/high-resolution/progress/:exportId for status.`
          : 'Export started. Poll /api/export/high-resolution/progress/:exportId for status.'
      });
      
    } catch (error) {
      console.error('[HighRes Async Export] Error:', error);
      res.status(500).json({
        success: false,
        error: 'Failed to start async export',
        message: error instanceof Error ? error.message : 'Unknown error'
      });
    }
  });
  ...
  /**
   * High-Resolution Export Progress Endpoint
   * 
   * Returns the current progress of an async export.
   */
  app.get('/api/export/high-resolution/progress/:exportId', (req, res) => {
    try {
      const { exportId } = req.params;
      
      const progress = highResExportService.getExportProgress(exportId);
      
      if (!progress) {
        return res.status(404).json({
          success: false,
          error: 'Export not found',
          message: 'The export may have expired or does not exist'
        });
      }
      
      // Return progress without the buffer (too large for JSON)
      const safeProgress: Omit<HighResExportProgress, 'result'> & { hasResult?: boolean } = {
        ...
        hasResult: progress.status === 'completed' && !!progress.result
      };
  ...
  /**
   * High-Resolution Export Download Endpoint
   * 
   * Downloads the result of a completed async export.
   */
  app.get('/api/export/high-resolution/download/:exportId', (req, res) => {
    try {
      const { exportId } = req.params;
      
      const result = highResExportService.getExportResult(exportId);
      ...
      res.setHeader('Content-Length', result.buffer.length);
      res.setHeader('X-Export-Width', result.width);
      res.setHeader('X-Export-Height', result.height);
      
      console.log(`[HighRes Export] Download: ${result.filename} (${(result.buffer.length / 1024 / 1024).toFixed(2)} MB)`);
      
      res.send(result.buffer);
      
      // Clean up after download
      highResExportService.cleanupExport(exportId);
      
    } catch (error) {
      console.error('[HighRes Export Download] Error:', error);
      res.status(500).json({
        success: false,
        error: 'Failed to download export',
        message: error instanceof Error ? error.message : 'Unknown error'
