...
export default function Sidebar({
  ...
  // Auto-save print configuration when it changes (debounced)
  // This effect watches for changes to the active artboard's print config
  const activeArtboardRef = artboards.find(a => a.id === activeArtboard);
  const printConfigJson = activeArtboardRef?.printConfig ? JSON.stringify(activeArtboardRef.printConfig) : null;
  
  useEffect(() => {
    if (!appSettingsDefaults || !hasRestoredSidebar.current || !activeArtboardRef) return;
    
    const printConfig = activeArtboardRef.printConfig || DEFAULT_PRINT_CONFIG;
    
    const timeoutId = setTimeout(() => {
      console.log('Saving app settings defaults:', {
        ...appSettingsDefaults,
        exportFormat,
        exportQuality,
        exportScale,
        ...
        exportMode,
        artboardName: activeArtboardRef.name,
        artboardWidth: activeArtboardRef.width,
        artboardHeight: activeArtboardRef.height,
        artboardBackgroundColor: activeArtboardRef.backgroundColor || '#ffffff',
        artboardGridColor: activeArtboardRef.gridColor || '#cccccc',
        artboardDisplayGrid: activeArtboardRef.displayGrid || false,
        artboardDisplayBorder: activeArtboardRef.displayBorder !== undefined ? activeArtboardRef.displayBorder : true,
        artboardDpi: activeArtboardRef.dpi ?? 72,
        artboardUnitType: activeArtboardRef.unitType ?? 'pixels',
        artboardDisplayName: activeArtboardRef.displayName !== false,
        artboardDisplayDimensions: activeArtboardRef.displayDimensions === true,
        artboardDisplayResolution: activeArtboardRef.displayResolution === true,
        sidebarCollapsed: isCollapsed,
    ...
    }, 1000); // 1 second debounce for print config changes
    
    return () => clearTimeout(timeoutId);
  }, [printConfigJson, exportFormat, exportQuality, exportScale, exportAutoScaleFromDpi, exportMode, isCollapsed]);
